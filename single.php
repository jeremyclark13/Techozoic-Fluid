...
<div id="content" class="<?php echo ((of_get_option( 'single_sidebar', '0' ) == "1" && $tech_sidebar == "unset") || $tech_sidebar == "on") ? "narrow" : "wide"; ?>column">
    ...
    if ( !empty( $tech_sing_ad_code ) && of_get_option( 'sing_ad_pos', 'above' ) == "above" ) {
        ...
        <div class="aligncenter">
            <?php
            $tech_sing_ad_code = stripslashes( $tech_sing_ad_code );
    ...
    if ( have_posts() ) {
        while ( have_posts() ) {
            the_post();
            ?>
            <?php tech_single_nav_links(); ?>
            <div <?php post_class( 'clear' ); ?> id="post-<?php the_ID(); ?>">
                <div class="toppost">
                    <a href="<?php echo home_url(); ?>" title="<?php printf( __( 'Go back to %s', 'techozoic' ), get_bloginfo( 'name' ) ); ?>" class="social homelink"></a>&nbsp;<?php
        ...
        <?php } ?>
                        ...
                        <?php the_tags( '<div class="single-tags">', ', ', '</div>' ); ?>
                        <div class="clear"></div>
